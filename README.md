...
The script pulls `ros:noetic-ros-base-focal`, clones PX4 v1.12.3, initializes PX4 submodules, installs explicit Gazebo Classic build dependencies, runs PX4's `Tools/setup/ubuntu.sh --no-nuttx` when available, builds `px4_sitl_default` and `sitl_gazebo`, extracts runtime artifacts, runs lightweight runtime checks, builds the Debian package, installs it in the same disposable container, and verifies that both ROS packages are discoverable by `rospack`.
